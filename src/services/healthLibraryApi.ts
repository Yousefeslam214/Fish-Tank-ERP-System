...
const unwrapArray = <T>(payload: unknown): T[] => {
  ...
  if (Array.isArray(record.healthLibraries))
    return record.healthLibraries as T[];
...
export const getHealthLibraryConditionId = (
  config: HealthLibraryConfiguration,
) =>
  String(
    config.condition_id || config.conditionId || config.condition || "",
  ).trim();
...
export const getConfigurableDiseaseLibraryTemplates = () =>
  getHealthLibraryTemplates().filter((template) =>
    DISEASE_LIBRARY_TEMPLATE_KEYS.includes(
      template.key as (typeof DISEASE_LIBRARY_TEMPLATE_KEYS)[number],
    ),
  );
...
const readLocalHealthLibraryConfigurations =
  (): HealthLibraryConfiguration[] => {
    ...
    try {
      ...
      return Array.isArray(parsed)
        ? parsed
            .map((config) => normalizeApiConfiguration(config))
            .filter((config) => getHealthLibraryConditionId(config))
        : [];
    } catch {
      return [];
    }
...
const writeLocalHealthLibraryConfigurations = (
  configs: HealthLibraryConfiguration[],
) => {
  if (!canUseLocalStorage()) return;
  window.localStorage.setItem(
    LOCAL_HEALTH_LIBRARY_KEY,
...
const upsertLocalHealthLibraryConfiguration = (
  ...
  const normalized = normalizeApiConfiguration({
    ...
    id:
      id ||
      getHealthLibraryConfigId(data) ||
      `local_${conditionId || "health"}_${Date.now()}`,
    _id: undefined,
    savedLocally: true,
  });
  const nextConfigs = [
    ...
    ...localConfigs.filter((config) => {
      const sameId =
        getHealthLibraryConfigId(config) &&
        getHealthLibraryConfigId(config) ===
          getHealthLibraryConfigId(normalized);
      const sameCondition =
        conditionId && getHealthLibraryConditionId(config) === conditionId;
    ...
    columnaris: [
      {
        ...
        recommendations: [
          "تقليل التعامل مع السمك",
          "مراجعة الكثافة وجودة المياه",
          "متابعة الجلد والفم خلال 24 ساعة",
        ],
      },
      {
        ...
        recommendations: [
          "عزل أدوات الحوض",
          "تقليل التغذية مؤقتاً",
          "بدء بروتوكول علاج Columnaris الذي يحدده الأدمن",
        ],
      },
      {
        ...
        recommendations: [
          "تدخل علاجي فوري",
          "رفع التهوية وتقليل الإجهاد",
          "متابعة يومية للنفوق والآفات",
        ],
      },
      {
        ...
        recommendations: [
          "عزل الدفعة المصابة",
          "إيقاف النقل أو الفرز",
          "مراجعة بيطرية عاجلة وتنفيذ تعليمات العلاج كاملة",
        ],
      },
    ],
    "bacterial-gill-disease": [
      ...
      },
      {
        ...
        recommendations: [
          "تشغيل دعم أكسجين إضافي",
          "تغيير مياه جزئي حسب تعليمات المزرعة",
          "بدء علاج الخياشيم الذي يحدده الأدمن",
        ],
      },
      {
        ...
        recommendations: [
          "تدخل طارئ في التهوية",
          "إيقاف التغذية مؤقتاً",
          "متابعة الحوض كل عدة ساعات",
        ],
      },
    ],
    streptococcosis: [
      ...
      },
      {
        ...
        recommendations: [
          "تدخل بيطري عاجل",
          "جمع النافق مرتين يومياً",
          "تنفيذ علاج Streptococcosis المكتوب من الأدمن",
        ],
      },
      {
        ...
        recommendations: [
          "عزل كامل للدفعة",
          "وقف أي إجهاد أو فرز",
          "متابعة مستمرة وتطبيق تعليمات العلاج فوراً",
        ],
      },
    ],
    aeromonas: [
      {
        ...
        recommendations: [
          "تحسين النظافة وإزالة المخلفات",
          "متابعة الشهية",
          "مراجعة أي إصابات جلدية",
        ],
      },
      {
        ...
        recommendations: [
          "تنظيف الحوض والأدوات",
          "تقليل التغذية حسب الشهية",
          "تطبيق توصيات علاج Aeromonas المكتوبة من الأدمن",
        ],
      },
      {
      ...
      },
      {
        ...
        recommendations: [
          "تدخل بيطري فوري",
          "رفع إجراءات التطهير",
          "تسجيل النفوق والأعراض يومياً",
        ],
      },
    ],
  };

export const getDefaultRiskLevelsForTemplate = (
  template?: HealthReportTemplate,
) =>
  cloneRiskLevels(
    DISEASE_DEFAULT_LEVELS[template?.key || ""] || DEFAULT_RISK_LEVELS,
  );
...
export const normalizeHealthLibraryRiskLevels = (
  ...
  const sourceLevels =
    Array.isArray(levels) && levels.length > 0 ? levels : fallbackLevels;
  return sourceLevels.map((level, index) => {
    const fallbackLevel = fallbackLevels[index] || createDefaultRiskLevel();
    const recommendations = cleanTextList(
      level.recommendations,
      fallbackLevel.recommendations,
    );
...
export const getDefaultDiseaseLibraryConfigurations = () =>
  getConfigurableDiseaseLibraryTemplates().map(
    buildDefaultDiseaseLibraryConfiguration,
  );
...
const resolveTemplateFromConfig = (config: HealthLibraryConfiguration) => {
  ...
  return (
    templates.find((template) => {
      ...
      const diseaseConditionKey = normalizeConditionKey(
        getDiseaseConditionId(template),
      );
...
export const mergeHealthLibraryConfigurationWithDefaults = (
  ...
  return {
    ...
    feedingGuidance: cleanTextList(
      config.feedingGuidance,
      defaults.feedingGuidance,
    ),
    recoveryChecklist: cleanTextList(
      config.recoveryChecklist,
      defaults.recoveryChecklist,
    ),
    preventiveMeasures: cleanTextList(
      config.preventiveMeasures,
      defaults.preventiveMeasures,
    ),
    quarantineAdvice: String(
      config.quarantineAdvice || defaults.quarantineAdvice || "",
    ),
    levels: normalizeHealthLibraryRiskLevels(config.levels, fallbackLevels),
  };
};

const conditionMatchesLabels = (
  config: HealthLibraryConfiguration,
  labels: string[],
) => {
...
const findConfigByConditionKeys = (
  configs: HealthLibraryConfiguration[],
  keys: string[],
...
export const mergeHealthLibraryConfigurationsWithDefaults = (
  ...
  const mergedDefaults = getDefaultDiseaseLibraryConfigurations().map(
    (defaultConfig) => {
      ...
      return mergeHealthLibraryConfigurationWithDefaults(
  ...
  const additionalConfigs = configs
    .filter(
      (config) =>
        !findConfigByConditionLabels(mergedDefaults, [
          getHealthLibraryConditionId(config),
        ]),
    )
...
const flattenImprovementTips = (tips?: Record<string, string[]>) =>
  Object.values(tips || {}).flatMap((items) =>
    Array.isArray(items) ? items : [],
  );

export const listHealthLibraryConfigurations = async (): Promise<
  HealthLibraryConfiguration[]
> => {
  const localConfigs = readLocalHealthLibraryConfigurations();
  try {
    const payload = await apiGet<unknown>("/tanks/health-library");
    const apiConfigs = unwrapArray<HealthLibraryConfiguration>(payload).map(
...
export const getHealthLibraryConfiguration = async (
  id: string,
): Promise<HealthLibraryConfiguration> => {
  const localConfig = readLocalHealthLibraryConfigurations().find(
    (config) => getHealthLibraryConfigId(config) === id,
  );
  if (localConfig) return localConfig;

  const payload = await apiGet<unknown>(`/tanks/health-library/${id}`);
  return normalizeApiConfiguration(
    unwrapPayload<HealthLibraryConfiguration>(payload),
  );
};

export const getHealthLibraryByCondition = async (
  conditionId: string,
): Promise<HealthLibraryConfiguration> => {
  const localConfig = readLocalHealthLibraryConfigurations().find(
  ...
  const payload = await apiGet<unknown>(
    `/tanks/health-library/condition/${encodeURIComponent(conditionId)}`,
  );
  return normalizeApiConfiguration(
    unwrapPayload<HealthLibraryConfiguration>(payload),
  );
...
export const createHealthLibraryConfiguration = async (
  ...
  try {
    const payload = await apiPost<unknown>(
      "/tanks/health-library",
      toApiConfigurationPayload(data),
    );
    return normalizeApiConfiguration(
      unwrapPayload<HealthLibraryConfiguration | undefined>(payload) || data,
    );
  } catch (error) {
...
export const updateHealthLibraryConfiguration = async (
  ...
  const { conditionId: _conditionId, ...payloadData } =
    toApiConfigurationPayload(data);
  try {
    const payload = await apiPatch<unknown>(
      `/tanks/health-library/${id}`,
      payloadData,
    );
    return normalizeApiConfiguration(
      unwrapPayload<HealthLibraryConfiguration | undefined>(payload) || {
        ...data,
        id,
      },
    );
...
export const deleteHealthLibraryConfiguration = async (
  id: string,
): Promise<void> => {
  if (id.startsWith("local_")) {
...
export const resolveHealthLibraryRecommendation = (
  ...
  const initialLabels = Array.isArray(conditionLabel)
    ? conditionLabel
    : [conditionLabel];
  ...
  const conditionConfig = findConfigByConditionLabels(
    mergedConfigs,
    conditionLabels,
  );
  const globalRiskConfig = findConfigByConditionKeys(mergedConfigs, [
    "fish_risk_levels",
    "animal_risk_levels",
  ]);
  const healthStatusConfig = findConfigByConditionKeys(mergedConfigs, [
    "fish_health_status",
  ...
  if (riskLevel && config) {
    ...
    return {
      ...
      range: {
        min: Number(riskLevel.range.min),
        max: Number(riskLevel.range.max),
      },
