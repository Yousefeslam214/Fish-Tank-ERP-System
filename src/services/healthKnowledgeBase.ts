...
const HEALTH_REPORT_TEMPLATES: HealthReportTemplate[] = [
  ...
  },
  {
    ...
    aliases: ['ammonia', 'stress', 'hypoxia', 'oxygen', 'ph stress', 'water quality'],
...
export const resolveHealthReportTemplate = (
  ...
  if (!normalized) {
    return HEALTH_REPORT_TEMPLATES.find((template) => template.key === 'generic-bacterial')!;
  }
  ...
  return matched ||
    HEALTH_REPORT_TEMPLATES.find((template) => template.key === 'generic-bacterial')!;
};
