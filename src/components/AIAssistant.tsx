...
export default function AIAssistant({ user }: AIAssistantProps) {
  ...
  const [healthHistoryByTank, setHealthHistoryByTank] = useState<
    Record<string, HealthCheckResponseDTO[]>
  >({});
  const [healthOverviewByTank, setHealthOverviewByTank] = useState<
    Record<string, TankHealthOverview>
  >({});
  ...
  const [improvingRecordId, setImprovingRecordId] = useState<string | null>(
    null,
  );
  ...
  const loadTankHealthHistory = useCallback(async () => {
    ...
    try {
      ...
      const overviewMap = result.overviews.reduce<
        Record<string, TankHealthOverview>
      >((acc, overview) => {
      ...
      setSelectedTankId((previous) => {
        if (previous && result.tanks.some((tank) => tank.id === previous))
          return previous;
        return result.tanks[0]?.id || "";
      });
    } catch (error) {
      toast.error(
        `Failed to load tank health history: ${(error as Error).message}`,
      );
    } finally {
  ...
  const loadAIHealth = useCallback(async () => {
    ...
    try {
      ...
      toast.error(
        `AI endpoint health check failed: ${(error as Error).message}`,
      );
      setAiHealth(null);
    } finally {
  ...
  const selectedBatch =
    selectedTankBatches.find((batch) => batch.id === selectedBatchId) || null;
  ...
  const matchingDiseaseHistory = useMemo(() => {
    ...
    return selectedTankHistory
      .filter((record) => {
        ...
        return labels.some(
          (label) =>
            disease && (disease.includes(label) || label.includes(disease)),
        );
      })
      .slice(0, 5);
  ...
  const handleMarkImproved = async (
    _batch: any,
    record: HealthCheckResponseDTO,
  ) => {
    ...
    try {
      ...
      toast.error(
        `Failed to save recovery report: ${(error as Error).message}`,
      );
    } finally {
    ...
    <div className="space-y-6 p-6">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <p className="text-gray-600">
            Live AI endpoint status, fixed robot reports, and batch-by-batch
            tank health history.
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          ...
          >
            <RefreshCw
              className={`mr-2 h-4 w-4 ${loadingAIHealth ? "animate-spin" : ""}`}
            />
            Check AI Link
          </Button>
          <Button
            variant="outline"
            onClick={() => void loadTankHealthHistory()}
            disabled={loadingHistory}
          >
            <RefreshCw
              className={`mr-2 h-4 w-4 ${loadingHistory ? "animate-spin" : ""}`}
            />
            Refresh Data
          </Button>
        </div>
      ...
      <Card>
        <CardContent className="flex flex-col gap-2 p-4 text-sm text-slate-600 lg:flex-row lg:items-center lg:justify-between">
          ...
          <span>
            {aiHealth?.timestamp
              ? `Last health check: ${formatDateTime(aiHealth.timestamp)}`
      ...
      <Tabs defaultValue="disease" className="space-y-4">
        ...
        <TabsContent value="disease" className="space-y-4">
          <Card>
            <CardHeader>
              <CardTitle className="text-sm">
                Generate Fixed AI Health Report
              </CardTitle>
              <p className="text-xs text-gray-600">
                Select the tank and batch, run the AI image analysis, then save
                the fixed report into the health-check history endpoint.
              </p>
            </CardHeader>
          ...
          <div className="grid grid-cols-1 gap-5 lg:grid-cols-[460px_minmax(0,760px)] lg:items-start lg:justify-start">
            <Card>
              <CardHeader>
                ...
                <p className="text-xs text-gray-600">
                  This uses the verified AI endpoint from the CRM app
                  integration.
                </p>
              </CardHeader>
              ...
              <CardContent className="max-h-[70vh] overflow-y-auto pr-1">
                {report ? (
                  <div className="space-y-4">
                    ...
                    <div className="rounded-lg border border-[#CBE7EC] bg-[#F4FBFC] p-4">
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          <p className="text-xs font-semibold uppercase tracking-wide text-[#088395]">
                            Top Prediction
                          </p>
                    ...
                    {analysis?.predictions?.length ? (
                      <div className="space-y-2 rounded-lg border border-slate-200 bg-white p-4">
                        ...
                        {analysis.predictions.slice(0, 4).map((prediction) => (
                          <div
                            key={`${prediction.class}-${prediction.confidence}`}
                            className="space-y-1"
                          >
                            <div className="flex items-center justify-between text-sm text-slate-700">
                              <span>
                                {humanizePredictionLabel(prediction.class)}
                              </span>
                              <span>
                                {confidenceToPercent(
                            ...
                            <div className="h-2 overflow-hidden rounded-full bg-slate-200">
                              <div
                                ...
                                style={{
                                  width: `${confidenceToPercent(prediction.confidence)}%`,
                                }}
                              />
                            </div>
                          </div>
                        ...
                        <div className="rounded-lg border border-slate-200 bg-white p-4">
                          <div className="flex flex-wrap items-center justify-between gap-3">
                            <div>
                              <p className="text-sm font-semibold text-slate-900">
                                Previous records for this disease
                              </p>
                              <p className="mt-1 text-xs text-slate-500">
                                Matched from saved health records for this tank
                                using the disease name and Health Library
                                aliases.
                              </p>
                            </div>
                            <Badge
                            ...
                            >
                              {matchingDiseaseHistory.length} record
                              {matchingDiseaseHistory.length === 1 ? "" : "s"}
                            </Badge>
                          </div>
                          {matchingDiseaseHistory.length === 0 ? (
                            <p className="mt-4 rounded-lg border border-dashed border-slate-200 bg-slate-50 p-4 text-sm text-slate-500">
                              No older saved records for this disease in the
                              selected tank yet.
                            </p>
                          ) : (
                  ...
                  <div className="flex min-h-[320px] flex-col items-center justify-center text-center text-gray-500">
                    ...
                    <p className="text-sm">
                      Upload an image and run AI analysis first.
                    </p>
                    <p className="mt-1 text-xs text-gray-400">
                      The result will be saved as a fixed report tied to the
                      detected disease template.
                    </p>
                  </div>
                )}
              </CardContent>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-sm">
                Recent Checks for Selected Tank
              </CardTitle>
            </CardHeader>
            <CardContent>
              {recentHistory.length === 0 ? (
                <p className="py-4 text-sm text-gray-500">
                  No health checks recorded for the selected tank yet.
                </p>
              ) : (
                  ...
                  {recentHistory.map((record) => (
                    ...
                    >
                      <div className="flex flex-wrap items-center gap-2">
                        <Badge
                          variant="outline"
                          className={getHealthStatusColor(record.healthStatus)}
                        >
                          {formatHealthStatus(record.healthStatus)}
                        </Badge>
                        <Badge
                      ...
                      <p className="mt-3 text-sm font-semibold text-slate-900">
                        {record.bacterialType || "No saved disease label"}
                      </p>
            ...
            <div className="grid grid-cols-1 gap-4 lg:grid-cols-[300px_1fr]">
                        ...
                        <div className="flex items-start justify-between gap-3">
                          <div>
                            ...
                            <p className="text-xs text-slate-500">
                              {tankBatches.length} batch
                              {tankBatches.length === 1 ? "" : "es"} •{" "}
                              {tankHistory.length} report
                              {tankHistory.length === 1 ? "" : "s"}
                            </p>
                          </div>
              ...
              {selectedTank ? (
                <HealthChecksTab
                  ...
                  onCreate={(batch) => {
                    setSelectedBatchForModal(
                      batch ?? selectedTankBatches[0] ?? null,
                    );
