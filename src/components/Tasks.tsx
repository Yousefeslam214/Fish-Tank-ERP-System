...
import { CheckCircle2, Clock3, ListChecks, RefreshCcw, Trash2 } from 'lucide-react';
...
export default function Tasks({ user }: TasksProps) {
  ...
  const [scope, setScope] = useState<'mine' | 'farm'>(canViewFarmTasks ? 'farm' : 'mine');
  ...
  const loadTasks = useCallback(async () => {
    ...
    try {
      const data =
        scope === 'mine'
          ? await getMyTasks()
          : await getFarmTasks(statusFilter === 'ALL' ? undefined : statusFilter);
      setTasks(data);
    } catch (err) {
    ...
    <div className="p-6 space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-gray-600">Track and complete operational tasks assigned from alerts and workflows.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card>
          ...
          <CardContent className="text-2xl font-semibold text-blue-700">{summary.OPEN}</CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm">In Progress</CardTitle>
          </CardHeader>
          <CardContent className="text-2xl font-semibold text-amber-700">{summary.IN_PROGRESS}</CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm">Done</CardTitle>
          </CardHeader>
          <CardContent className="text-2xl font-semibold text-emerald-700">{summary.DONE}</CardContent>
        </Card>
      </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          ...
          {!loading && error && (
            <div className="text-sm text-red-700">Failed to load tasks: {error}</div>
          )}
          ...
          {!loading &&
            ...
            filteredTasks.map((task) => {
              ...
              const canCancel = canCancelTask && task.status !== 'DONE' && task.status !== 'CANCELLED';
