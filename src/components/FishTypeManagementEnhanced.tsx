...
const CRITICAL_PARAMETER_OPTIONS = [
  "Temperature",
  "Dissolved Oxygen",
  "pH",
  "Ammonia",
...
const cloneFeedingRateMatrix = (
  matrix: FeedingRateMatrix,
): FeedingRateMatrix => ({
  weight_ranges: matrix.weight_ranges.map((range) => ({
    min: range.min,
    max: range.max,
  })),
...
const normalizeFeedingRateMatrix = (
  matrix: FeedingRateMatrix,
): FeedingRateMatrix => {
  const temperatures =
    matrix.temperatures.length > 0 ? [...matrix.temperatures] : [20, 24, 28];
  const weightRanges =
    matrix.weight_ranges.length > 0
      ? matrix.weight_ranges.map((range) => ({
          min: range.min ?? 0,
          max: range.max ?? 0,
        }))
      : [{ min: 0, max: 10 }];
...
const calculateDistributionTotal = (
  entries: ExpectedGradeDistributionEntry[],
): number =>
  entries.reduce(
    (sum, entry) =>
      sum + (Number.isFinite(entry.percentage) ? entry.percentage : 0),
    0,
  );
...
const validateForm = (form: FishTypeFormState): string | null => {
  ...
  if (
    form.feedingRateMatrix.weight_ranges.length === 0 ||
    form.feedingRateMatrix.temperatures.length === 0
  ) {
    return "Feeding matrix must include at least one weight range and one temperature.";
  }

  const invalidRange = form.feedingRateMatrix.weight_ranges.find(
    (range) =>
  ...
  if (
    invalidRateShape ||
    form.feedingRateMatrix.rates.length !==
      form.feedingRateMatrix.weight_ranges.length
  ) {
  ...
  const invalidProtein = form.proteinRequirements.some(
    (rule) =>
      !Number.isFinite(rule.proteinPercentage) || rule.proteinPercentage <= 0,
  );
  if (invalidProtein) {
...
const toUpsertPayload = (form: FishTypeFormState): FishTypeUpsertPayload => ({
  ...
  allowedFoodTypeIds:
    form.allowedFoodTypeIds.length > 0 ? form.allowedFoodTypeIds : undefined,
  isActive: form.isActive,
});

export default function FishTypeManagementEnhanced({
  user,
  selectedFarm,
}: FishTypeManagementProps) {
  ...
  const [formState, setFormState] = useState<FishTypeFormState>(
    getDefaultFormState(),
  );
  const [gradePricingOptions, setGradePricingOptions] = useState<
    FishGradePricingRecord[]
  >([]);
  ...
  const [feedingRateResult, setFeedingRateResult] =
    useState<FeedingRateResult | null>(null);
  // Meal frequency state removed
  const [proteinRequirementResult, setProteinRequirementResult] =
    useState<ProteinRequirementResult | null>(null);
  ...
  const loadFishTypes = async (silent = false) => {
    ...
    try {
      ...
      setErrorMessage(
        error instanceof Error ? error.message : "Failed to load fish types.",
      );
      setFishTypes([]);
    } finally {
      setIsLoading(false);
  ...
  const loadFoodTypes = async () => {
    ...
    try {
      ...
      setErrorMessage(
        error instanceof Error ? error.message : "Failed to load food types.",
      );
      setFoodTypes([]);
    } finally {
    ...
    try {
      const [detail, pricing] = await Promise.all([
        ...
        getPricingByFishType(fishTypeId).catch((error) => {
          setGradePricingError(
            error instanceof Error
              ? error.message
              : "Failed to load grade pricing.",
          );
          return [];
        }),
      ]);
      ...
      setFormError(
        error instanceof Error
          ? error.message
          : "Failed to load fish type details.",
      );
    } finally {
      setIsSaving(false);
  ...
  const updateFeedingMatrix = (
    updater: (matrix: FeedingRateMatrix) => void,
  ) => {
    setFormState((previous) => {
  ...
  const addFeedingTemperature = () => {
    updateFeedingMatrix((matrix) => {
      const nextTemperature =
        matrix.temperatures.length > 0
          ? matrix.temperatures[matrix.temperatures.length - 1] + 2
          : 24;
  ...
  const removeFeedingTemperature = (index: number) => {
    updateFeedingMatrix((matrix) => {
      ...
      matrix.temperatures = matrix.temperatures.filter(
        (_, tempIndex) => tempIndex !== index,
      );
      matrix.rates = matrix.rates.map((row) =>
        row.filter((_, tempIndex) => tempIndex !== index),
      );
    });
  };
  ...
  const removeWeightRange = (index: number) => {
    updateFeedingMatrix((matrix) => {
      ...
      matrix.weight_ranges = matrix.weight_ranges.filter(
  ...
  const updateProteinRequirement = (
    index: number,
    field: keyof ProteinRequirementRule,
    value: number | null,
  ) => {
    setFormState((previous) => ({
      ...previous,
      proteinRequirements: previous.proteinRequirements.map(
        (rule, ruleIndex) =>
  ...
  const addProteinRequirement = () => {
    setFormState((previous) => ({
      ...
      proteinRequirements: [
        ...
        { minWeight: 0, maxWeight: null, proteinPercentage: 30 },
      ],
    }));
  };
    ...
    setFormState((previous) => ({
      ...
      proteinRequirements:
        previous.proteinRequirements.length === 1
          ? previous.proteinRequirements
          : previous.proteinRequirements.filter(
  ...
  const addExpectedDistributionRow = () => {
    setFormState((previous) => ({
      ...previous,
      expectedGradeDistribution: [
        ...previous.expectedGradeDistribution,
        { gradePricingId: "", percentage: 0 },
      ],
    }));
  };
  ...
  const removeExpectedDistributionRow = (index: number) => {
    setFormState((previous) => ({
      ...previous,
      expectedGradeDistribution: previous.expectedGradeDistribution.filter(
  ...
  const updateExpectedDistributionRow = (
    ...
    setFormState((previous) => ({
      ...previous,
      expectedGradeDistribution: previous.expectedGradeDistribution.map(
        (row, rowIndex) =>
  ...
  const saveFishType = async () => {
    ...
    try {
      ...
      setFormError(
        error instanceof Error ? error.message : "Failed to save fish type.",
      );
    } finally {
      setIsSaving(false);
    }
  ...
  const runCalculators = async () => {
    ...
    try {
      ...
      setCalculatorError(
        error instanceof Error ? error.message : "Failed to run calculators.",
      );
    } finally {
      ...
      <div className="flex-1 p-6 space-y-6 overflow-auto">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-600 text-sm mt-1">
              Live backend integration for fish type profiles and calculators
            </p>
          </div>
          <div className="flex gap-2">
            <Button
              variant="outline"
              onClick={() => void loadFishTypes(true)}
            ...
            {fishTypes.map((fishType) => (
              <Card key={fishType.id} className="bg-white shadow-sm">
                ...
                <CardContent className="space-y-3 text-sm">
                  <p>
                    Temp: {fishType.tempMin} - {fishType.tempMax} °C
                  </p>
                  <p>
                    DO: {fishType.doMin} - {fishType.doSafe} mg/L
                  </p>
                  <p>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
              <div className="md:col-span-2">
                ...
                <select
                  ...
                  onChange={(event) =>
                    setCalculatorFishTypeId(event.target.value)
                  }
                >
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(event) =>
                    setCalculatorWeight(Number(event.target.value) || 0)
                  }
                />
              </div>
              <div>
                ...
                <Input
                  ...
                  onChange={(event) =>
                    setCalculatorTemperature(Number(event.target.value) || 0)
                  }
                />
              </div>
            </div>
            <Button
              className="bg-[#088395] hover:bg-[#0A4D68]"
              onClick={() => void runCalculators()}
              disabled={isRunningCalculator}
            >
              {isRunningCalculator ? (
                <Loader2 className="w-4 h-4 mr-2 animate-spin" />
              ) : null}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <Card className="bg-gray-50">
                ...
                <CardContent className="text-sm">
                  {feedingRateResult
                    ? `${feedingRateResult.feedingRatePercentage}% body weight/day`
                    : "Run calculator"}
                </CardContent>
              </Card>
              ...
              <Card className="bg-gray-50">
                ...
                <CardContent className="text-sm">
                  {proteinRequirementResult
                    ? `${proteinRequirementResult.proteinPercentage}% protein`
                    : "Run calculator"}
                </CardContent>
              </Card>
            </div>
      ...
      >
        <DialogContent className="max-w-[95vw] w-[860px] max-h-[95vh] overflow-y-auto p-0 bg-white border border-[#BFD2DA] shadow-2xl">
          <div className="px-6 pt-6 pb-4 border-b border-[#C8D9E0] bg-white sticky top-0 z-10">
            <DialogHeader className="gap-0">
              ...
              <DialogDescription className="sr-only">
                Configure fish type profile, water quality, feeding matrix,
                protein rules, and food compatibility.
              </DialogDescription>
            </DialogHeader>
            {formError && (
          ...
          >
            ...
            <TabsContent
              value="basic"
              className="space-y-4 mt-0 border border-[#D3E1E8] rounded-lg bg-white p-4"
            >
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <Label
                    htmlFor="fish-type-name"
                ...
                <div>
                  <Label
                    htmlFor="fish-type-scientific-name"
                    className="text-sm font-medium"
                  >
                    Scientific Name *
                  </Label>
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(event) =>
                      setFormState((previous) => ({
                        ...previous,
                        targetSGR: toNumber(event.target.value),
                      }))
                    }
                  />
                </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                  {CRITICAL_PARAMETER_OPTIONS.map((parameter) => {
                    const selected =
                      formState.criticalParameters.includes(parameter);
                    return (
                      <button
                        type="button"
                        key={parameter}
                        className={`px-3 py-2 rounded-lg border text-sm text-left ${
                          selected
                            ? "bg-[#E0F4F5] border-[#088395] text-[#0A4D68]"
            ...
            <TabsContent
              value="water"
              className="space-y-6 mt-0 border border-[#D3E1E8] rounded-lg bg-white p-4"
            >
              <div>
                <h3 className="text-lg font-semibold text-[#1F2937] mb-4">
                  Water Quality Thresholds (Min - Max)
                </h3>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {[
                    ...
                    <div
                      key={label}
                      className="p-3 border rounded-lg bg-gray-50/50 space-y-3"
                    >
                      <h4 className="text-sm font-bold text-[#0A4D68] border-b pb-1">
                        {label}
                      </h4>
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          ...
                          <Input
                            ...
                            value={
                              formState[
                                minKey as keyof FishTypeFormState
                              ] as number
                            }
                            onChange={(e) =>
                              setFormState((prev) => ({
                                ...prev,
                                [minKey]: toNumber(e.target.value),
                              }))
                            }
                        ...
                        <div>
                          ...
                          <Input
                            ...
                            value={
                              formState[
                                maxKey as keyof FishTypeFormState
                              ] as number
                            }
                            onChange={(e) =>
                              setFormState((prev) => ({
                                ...prev,
                                [maxKey]: toNumber(e.target.value),
                              }))
                            }
                            className="h-9 bg-white"
                          />
              ...
              <div>
                ...
                <div className="space-y-3">
                    ...
                    {[
                      ...
                      <div key={key}>
                        ...
                        <Input
                          ...
                          value={
                            formState[key as keyof FishTypeFormState] as number
                          }
                          onChange={(event) =>
                            setFormState((previous) => ({
                              ...previous,
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {[
                      {
                        key: "targetWeightForHarvest",
                        label: "Target Weight For Harvest (kg)",
                      },
                      {
                        key: "defaultMarketPrice",
                        label: "Default Market Price (EGP/kg)",
                      },
                    ].map(({ key, label }) => (
                      <div key={key}>
                        <Label className="text-xs font-medium text-gray-700">
            ...
            >
              ...
              <Card className="bg-white border border-[#D3E1E8]">
                <CardContent className="pt-4 space-y-3">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {formState.feedingRateMatrix.temperatures.map(
                      (temperature, index) => (
                        <div
                          key={`temperature-${index}`}
                          className="flex items-center gap-1 border rounded-md bg-gray-50 px-2 py-1"
                        >
                          <Input
                            type="number"
                            ...
                            onChange={(event) =>
                              updateFeedingMatrix((matrix) => {
                                matrix.temperatures[index] = toNumber(
                                  event.target.value,
                                );
                              })
                            }
                          ...
                          <Button
                            ...
                            disabled={
                              formState.feedingRateMatrix.temperatures.length <=
                              1
                            }
                          >
              ...
              <div className="overflow-x-auto border rounded-lg bg-white">
                <table className="w-full min-w-[720px] text-sm">
                  ...
                  <tbody>
                    {formState.feedingRateMatrix.weight_ranges.map(
                      (range, weightIndex) => (
                        <tr
                          key={`weight-range-${weightIndex}`}
                        ...
                        >
                          <td className="px-3 py-2 border-b">
                            <div className="flex items-center gap-2">
                              <Input
                                ...
                                onChange={(event) =>
                                  updateFeedingMatrix((matrix) => {
                                    matrix.weight_ranges[weightIndex].min =
                              ...
                              <Input
                                ...
                                onChange={(event) =>
                                  updateFeedingMatrix((matrix) => {
                                    matrix.weight_ranges[weightIndex].max =
                                      toNumber(event.target.value);
                                  })
                                }
                              />
                          ...
                          {formState.feedingRateMatrix.temperatures.map(
                            (_, temperatureIndex) => (
                              <td
                                key={`rate-cell-${weightIndex}-${temperatureIndex}`}
                              ...
                              >
                                <Input
                                  ...
                                  value={
                                    formState.feedingRateMatrix.rates[
                                      weightIndex
                                    ]?.[temperatureIndex] ?? 0
                                  }
                                  onChange={(event) =>
                                    updateFeedingMatrix((matrix) => {
                                      matrix.rates[weightIndex][
                                        temperatureIndex
                                      ] = toNumber(event.target.value);
                                    })
                                  }
                          ...
                          <td className="px-3 py-2 border-b text-center">
                            <Button
                              ...
                              disabled={
                                formState.feedingRateMatrix.weight_ranges
                                  .length <= 1
                              }
                            >
                ...
                <CardContent className="space-y-2">
                  {formState.proteinRequirements.length === 0 ? (
                    ...
                    formState.proteinRequirements.map((rule, index) => (
                      <div
                        key={`protein-rule-${index}`}
                        className="grid grid-cols-1 sm:grid-cols-[1fr_1fr_1fr_auto] gap-2 items-end p-2 border rounded-md bg-gray-50"
                      >
                        <div>
                          <Label className="text-xs font-medium text-gray-700">
                            Min Weight (g)
