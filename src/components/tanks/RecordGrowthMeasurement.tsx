...
import { Scale, CheckCircle, Activity } from "lucide-react";
...
export default function RecordGrowthMeasurement({
  ...
  useEffect(() => {
    if (open && measurement) {
      setFormData({
        measuredAt: new Date(
          measurement.measuredAt || measurement.date || measurement.timestamp,
        )
          .toISOString()
          .split("T")[0],
            ...
            <Card className="bg-[#E0F4F5] border-[#088395]">
              <CardContent className="p-4">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  {batch.lastWeight && (
                    <div>
                      <span className="text-gray-600">
                        {t("growthMeasurement.lastWeight")}:
                      </span>
                  ...
                  <div>
                    <span className="text-gray-600">
                      {t("growthMeasurement.currentCount")}:
                    </span>
                    <span className="font-medium ml-2">
                      {batch.currentCount} {t("growthMeasurement.fish")}
                    </span>
                  </div>
                </div>
              </CardContent>
                  ...
                  <div className="space-y-4 text-sm">
                    <div className="grid grid-cols-2 gap-4">
                      <div className="flex flex-col">
                        <span className="text-gray-600">
                          {t("growthMeasurement.averageWeight")}
                        </span>
                        <span className="font-bold text-lg">
                          {(
                            successData.averageWeightGrams ||
                            successData.averageWeight ||
                            0
                          ).toFixed(1)}
                          g
                        </span>
                      ...
                      <div className="flex flex-col">
                        <span className="text-gray-600">
                          {t("growthMeasurement.weightGain")}
                        </span>
                        <span className="font-bold text-lg text-[#10B981]">
                          +
                          {(
                            successData.weightGainGrams ||
                            successData.weightGain ||
                            0
                          ).toFixed(1)}
                          g
                        </span>
