...
export function HealthChecksTab({
  ...
  const batchEntries = useMemo(
    () =>
      tankBatches.map((batch) => {
        ...
        const latestHealthyRecord = history.find((record) => !isActiveHealthConcern(record)) || null;
  ...
  useEffect(() => {
    const firstBatchWithHistory = batchEntries.find((entry) => entry.history.length > 0)?.batch.id;
    const fallbackBatchId = firstBatchWithHistory || batchEntries[0]?.batch.id || '';
    setSelectedBatchId((previous) =>
      previous && batchEntries.some((entry) => entry.batch.id === previous) ? previous : fallbackBatchId,
    );
  }, [batchEntries]);
  ...
  const selectedBatchEntry = batchEntries.find((entry) => entry.batch.id === selectedBatchId) || batchEntries[0] || null;
  ...
  const selectedRecord = selectedBatchHistory.find((record) => record.id === selectedRecordId) || selectedBatchHistory[0] || null;
  const selectedTemplate = resolveHealthReportTemplate(selectedRecord?.bacterialType);

  const totalChecks = healthChecks.length;
  const activeBatches = batchEntries.filter((entry) => entry.requiresAttention).length;
  const recoveredBatches = batchEntries.filter((entry) => entry.isRecovered).length;
    ...
    <div className="space-y-4 pt-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-lg font-bold">
              {latestRecord ? formatHealthStatus(latestRecord.healthStatus) : 'No Data'}
            </div>
            <p className="mt-1 text-xs text-gray-500">
              {latestRecord ? formatDateTime(latestRecord.checkedAt) : 'No health history yet'}
            </p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <CardTitle className="text-lg">Batch-by-Batch Health History</CardTitle>
            <p className="mt-1 text-sm text-gray-500">
              Each batch keeps its own separate AI report list. Reports are fixed and linked to the detected disease template.
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            ...
            <Button
              ...
              onClick={() => onCreate(selectedBatchEntry?.batch || tankBatches[0])}
            >
        ...
        <CardContent className="space-y-4">
          ...
          {tankBatches.length === 0 ? (
            <div className="rounded-xl border border-dashed border-slate-300 bg-slate-50 px-6 py-12 text-center">
              ...
              <p className="text-base font-semibold text-slate-700">No batches available for this tank.</p>
            </div>
          ) : (
            <>
              <div className="flex flex-wrap gap-2">
                {batchEntries.map((entry) => (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <p className="text-sm font-semibold text-slate-900">{getBatchLabel(entry.batch)}</p>
                      {entry.latestRecord ? (
                        <Badge variant="outline" className={getHealthStatusColor(entry.latestRecord.healthStatus)}>
                    ...
                    <p className="mt-1 text-xs text-slate-500">
                      {entry.history.length} report{entry.history.length === 1 ? '' : 's'}
                    </p>
                  </button>
                ))}
              </div>
                ...
                <div className="grid gap-4 xl:grid-cols-[320px_1fr]">
                  <div className="space-y-3 rounded-3xl border border-slate-200 bg-slate-50 p-4">
                    <div className="flex items-center justify-between gap-3">
                      <div>
                        <p className="text-sm font-semibold text-slate-900">{getBatchLabel(selectedBatchEntry.batch)}</p>
                        <p className="text-xs text-slate-500">
                          {selectedBatchHistory.length} saved report{selectedBatchHistory.length === 1 ? '' : 's'}
                        </p>
                      </div>
                    ...
                    {selectedBatchEntry.latestActiveRecord && onMarkImproved && selectedBatchEntry.requiresAttention && (
                      <Button
                        className="w-full bg-emerald-600 hover:bg-emerald-700"
                        onClick={() => onMarkImproved(selectedBatchEntry.batch, selectedBatchEntry.latestActiveRecord!)}
                        disabled={improvingRecordId === selectedBatchEntry.latestActiveRecord.id}
                      >
                        <CheckCircle2 className="mr-2 h-4 w-4" />
                        {improvingRecordId === selectedBatchEntry.latestActiveRecord.id ? 'Saving recovery...' : 'Mark Batch Improved'}
                      </Button>
                    )}
                        ...
                        >
                          <div className="flex items-center justify-between gap-3">
                            <div>
                              ...
                              <p className="text-xs text-slate-500">{formatDateTime(record.checkedAt)}</p>
                            </div>
                          ...
                          <div className="mt-2 flex flex-wrap gap-2 text-xs text-slate-500">
                            ...
                            {record.bacterialLoadPercentage != null && (
                              <span>{Number(record.bacterialLoadPercentage).toFixed(1)}%</span>
                            )}
                          </div>
                        </button>
                      ))
                  ...
                  {selectedRecord ? (
                    ...
                    <div className="flex min-h-[360px] flex-col items-center justify-center rounded-3xl border border-dashed border-slate-300 bg-slate-50 p-8 text-center">
                      ...
                      <p className="text-sm text-slate-600">Select a report from the batch list to review the full fixed report.</p>
                    </div>
                  )}
                </div>
              ) : null}
