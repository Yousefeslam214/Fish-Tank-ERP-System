...
const taskMatchesTank = (task: TaskItem, tank: any): boolean => {
  ...
  const tankName = String(tank?.name ?? "")
    .trim()
    .toLowerCase();
  const tankIdPrefix = String(tank?.id ?? "")
    .trim()
    .toLowerCase()
    .split("-")[0];
...
export function TankTasksTab({ user, tank }: TankTasksTabProps) {
  ...
  const loadTasks = useCallback(async () => {
    ...
    try {
      ...
      const sourceTasks = canViewFarmTasks
      ...
      const scoped = statusFiltered.filter((task) =>
        taskMatchesTank(task, tank),
      );
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="text-sm text-gray-600">
            Tank:{" "}
            <span className="font-medium">{tank?.name || "Unknown Tank"}</span>
          </p>
        </div>
            ...
            tasks.map((task) => {
              ...
              return (
                <div key={task.id} className="rounded-lg border p-3">
                  ...
                  <p className="text-md text-black-700 mb-3">
