        ...
        <Card>
          ...
          <CardContent>
            {(() => {
              ...
              const assessmentRaw = firstBatchId
                ? batchAssessments[firstBatchId]
                : null;
              ...
              const displayWq = {
                overall:
                  assessment?.status ||
                  currentTank.waterQuality?.overall ||
                  "Unknown",
                temp:
                  params.temperature?.value ??
                  currentTank.waterQuality?.temp?.value ??
                  currentTank.waterQuality?.temperature ??
                  0,
                do:
                  params.dissolvedOxygen?.value ??
                  currentTank.waterQuality?.do?.value ??
                  currentTank.waterQuality?.dissolvedOxygen ??
                  0,
                ph:
                  params.pH?.value ??
                  currentTank.waterQuality?.ph?.value ??
                  currentTank.waterQuality?.phValue ??
                  "–",
                nh3:
                  params.ammonia?.value ??
                  currentTank.waterQuality?.nh3?.value ??
                  currentTank.waterQuality?.ammonia ??
                  0,
                status: {
                  temp:
                    params.temperature?.status ||
                    currentTank.waterQuality?.temp?.status ||
                    "OPTIMAL",
                  do:
                    params.dissolvedOxygen?.status ||
                    currentTank.waterQuality?.do?.status ||
                    "OPTIMAL",
                  ph:
                    params.pH?.status ||
                    currentTank.waterQuality?.ph?.status ||
                    "OPTIMAL",
                  nh3:
                    params.ammonia?.status ||
                    currentTank.waterQuality?.nh3?.status ||
                    "OPTIMAL",
                },
              };
              ...
              return hasRealData ? (
                <div className="bg-gray-50 p-4 rounded-lg">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <Badge
                      className={`${displayWq.overall.toUpperCase() === "OPTIMAL" ? "bg-[#10B981]" : displayWq.overall.toUpperCase() === "CRITICAL" ? "bg-red-500" : "bg-[#F59E0B]"} text-white font-bold`}
                    >
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <WaterParameter
                      name="Temperature"
                      value={displayWq.temp + "°C"}
                      status={displayWq.status.temp}
                    />
                    <WaterParameter
                      name="DO"
                      value={displayWq.do + " mg/L"}
                      status={displayWq.status.do}
                    />
                    <WaterParameter
                      name="pH"
                      value={displayWq.ph}
                      status={displayWq.status.ph}
                    />
                    <WaterParameter
                      name="NH₃"
                      value={
                        (typeof displayWq.nh3 === "number"
                          ? displayWq.nh3.toFixed(3)
                          : displayWq.nh3) + " mg/L"
                      }
                      status={displayWq.status.nh3}
                    />
                  </div>
                ...
                <div className="bg-gray-50 p-8 rounded-lg text-center border border-dashed flex flex-col items-center justify-center">
                  ...
                  <p className="text-sm text-gray-500 italic">
                    No recent water quality readings for this tank
                  </p>
                </div>
              );
            })()}
          ...
          <CardContent>
            {currentTank.feeding ? (
              <div className="bg-gray-50 p-4 rounded-lg space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-medium">
                    {currentTank.feeding.todayMeals}/
                    {currentTank.feeding.totalMeals}
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-sm text-gray-600">Total Fed Today</span>
                  <span className="font-medium">
                    {currentTank.feeding.todayFed} kg
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-sm text-gray-600">Recommended</span>
                  <span className="font-medium">
                    {currentTank.feeding.recommended} kg
                  </span>
                </div>
                <Progress
                  value={
                    currentTank.feeding.recommended > 0
                      ? (currentTank.feeding.todayFed /
                          currentTank.feeding.recommended) *
                        100
                      : 0
                  }
                  className="h-2"
                ...
                <div className="flex items-center justify-between text-xs">
                  ...
                  <span
                    className={`font-medium ${currentTank.feeding.todayFed < currentTank.feeding.recommended ? "text-yellow-600" : "text-green-600"}`}
                  >
                    {currentTank.feeding.recommended > 0
                      ? Math.round(
                          (currentTank.feeding.todayFed /
              ...
              <div className="bg-gray-50 p-8 rounded-lg text-center border border-dashed flex flex-col items-center justify-center">
                ...
                <p className="text-sm text-gray-500 italic">
                  No feeding plan active for this tank
                </p>
              </div>
            )}
          </CardContent>
