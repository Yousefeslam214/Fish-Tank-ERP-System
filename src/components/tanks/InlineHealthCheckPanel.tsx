    ...
    <div className="w-full rounded-lg border border-[#B9E0E7] bg-white shadow-sm">
      <div className="border-b border-slate-200 p-4">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              Select the batch, upload a fish image, then save the verified report.
            </p>
          </div>
          {onClose && (
        ...
        <div className="mt-4 grid gap-2 sm:grid-cols-3">
          {[
            ['1', 'Target batch', selectedBatch ? getBatchLabel(selectedBatch) : 'Not selected'],
      ...
      <div className="grid lg:grid-cols-[380px_minmax(0,1fr)]">
        <div className="space-y-5 border-b border-slate-200 p-4 lg:border-b-0 lg:border-r">
          <div>
            <label className="mb-2 block text-sm font-semibold text-slate-900">Save target</label>
            <Select value={selectedBatchId} onValueChange={setSelectedBatchId} disabled={tankBatches.length === 0}>
            ...
            <p className="mt-2 text-xs text-slate-500">The report will be saved under this batch history.</p>
          </div>
          ...
          <div className="rounded-lg border border-slate-200 bg-slate-50 p-3">
            ...
            <div className="aspect-[4/3] overflow-hidden rounded-lg border border-slate-200 bg-white">
              {previewUrl ? (
                ...
                >
                  <img src={previewUrl} alt="Fish preview" className="h-full w-full object-contain p-3" />
                </button>
              ) : (
                <label
                ...
                >
                  ...
                  <p className="text-sm font-semibold text-slate-800">Choose fish image</p>
                  <p className="mt-1 text-xs text-slate-500">JPG, PNG, or camera photo</p>
                </label>
              )}
            </div>
          </div>
          ...
          <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2">
            ...
            <Button type="button" variant="outline" className="h-11 sm:col-span-2 lg:col-span-1 xl:col-span-2" onClick={resetAll}>
          ...
          {expandedImage && (
            <div className="overflow-hidden rounded-lg border border-slate-200 bg-white p-3">
              ...
              <div className="flex h-[260px] items-center justify-center overflow-auto rounded-lg bg-slate-50">
                <img src={expandedImage.src} alt={expandedImage.title} className="max-h-[240px] w-auto max-w-full object-contain p-2" />
              </div>
            </div>
          )}
        </div>
          ...
          <div className="grid gap-4 xl:grid-cols-[minmax(0,320px)_1fr]">
            <div className="rounded-lg border border-slate-200 bg-slate-50 p-3">
              <div className="mb-3 flex items-center justify-between">
                ...
                {analysis && <Badge className="bg-[#0A4D68] text-white">{report?.topPredictionDisplay || 'Analyzed'}</Badge>}
              </div>
              <div className="aspect-[4/3] overflow-hidden rounded-lg border border-slate-200 bg-white">
                {annotatedImageSrc ? (
                  <button
                  ...
                  >
                    <img src={annotatedImageSrc} alt="Annotated result" className="h-full w-full object-contain p-3" />
                  </button>
                ) : (
                  <div className="flex h-full w-full flex-col items-center justify-center p-6 text-center">
                    <Bot className="mb-3 h-9 w-9 text-slate-300" />
                    <p className="text-sm font-semibold text-slate-700">Run AI to show output</p>
                  </div>
                )}
              </div>
            </div>
              ...
              {report ? (
                <div className="space-y-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    <Button className="bg-[#088395] hover:bg-[#0A4D68]" onClick={handleSave} disabled={isSaving || !canSaveReport}>
                ...
                <div className="flex min-h-[320px] flex-col items-center justify-center rounded-lg border border-dashed border-slate-300 bg-slate-50 p-6 text-center">
                  ...
                  <p className="text-base font-semibold text-slate-800">Report will appear here</p>
                  <p className="mt-2 max-w-md text-sm text-slate-500">
                    Results, health library match, treatment actions, and save button stay in one place.
                  </p>
                </div>
              )}
            </div>
