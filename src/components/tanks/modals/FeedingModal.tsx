...
import {
  getFoodTypesBySpecies,
  FoodType,
} from "../../../services/foodTypesApi";
...
export function FeedingModal({
      ...
      const fetchBatchRequirement = async () => {
        ...
        try {
          ...
          const res = await apiGet<any>(
            `/tanks/feeding-records/calculation/batch/${selectedBatchId}`,
          );
          ...
          const selectedBatch = tankBatches.find(
            (b) => b.id.toString() === selectedBatchId,
          );
          if (selectedBatch?.feedingPlan) {
            console.log("Falling back to feedingPlan from batch props");
            setBatchRequirement({
              recommendedAmount: parseFloat(
                selectedBatch.feedingPlan.dailyFeedingAmount || "0",
              ),
              totalRecommended: parseFloat(
                selectedBatch.feedingPlan.dailyFeedingAmount || "0",
              ),
  ...
  useEffect(() => {
    if (
      batchRequirement?.recommendedFoodType?.id &&
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await apiPost<any>(
        `/tanks/feeding-records/${selectedBatchId}`,
        payload,
      );
      ...
      if (res && res.success === false) {
        toast.error(
          "Failed to record feeding: " + (res.message || "Validation error"),
        );
        setIsSaving(false);
        return;
      }
      ...
      try {
        ...
        const targetUserIds =
          assignedUserIds.length > 0 ? assignedUserIds : [user.id];
        ...
        console.warn(
          "Feeding record saved, but failed to create task(s):",
          taskErr,
        );
      }
      ...
      toast.success("Feeding record saved and tasks synchronized", {
        description:
          createdTasks.length > 0
            ? `${createdTasks.length} task(s) created and assigned.`
            : undefined,
        action: {
          label: "View Tasks",
          onClick: () => {
        ...
        <div className="bg-gradient-to-r from-[#0A4D68] to-[#088395] p-6 text-white">
          <DialogHeader className="text-left">
            ...
            <div className="flex flex-wrap items-center gap-2 mt-2 opacity-90">
              ...
              <span className="flex items-center gap-1 bg-white/20 px-2 py-0.5 rounded text-xs font-medium">
                <Fish className="w-3 h-3" /> Batch #
                {selectedBatchId?.slice(-6).toUpperCase() || "---"}
              </span>
            </div>
          </DialogHeader>
        </div>
        ...
        <div className="p-6 space-y-6">
          <div className="bg-[#E0F4F5]/40 border border-[#088395]/20 rounded-2xl p-5 relative overflow-hidden group">
            <div className="flex items-center justify-between gap-2 font-bold mb-4">
              ...
              {batchRequirement?.safetyStatus &&
                batchRequirement.safetyStatus !== "SAFE" && (
                  <Badge
                    variant="outline"
                    className={`text-[10px] uppercase font-bold py-0 h-5 ${
                      batchRequirement.safetyStatus === "WARNING"
            ...
            <div className="grid grid-cols-2 gap-x-6 gap-y-4">
              <div className="space-y-1">
                <p className="text-[#0A4D68]/60 text-[10px] uppercase font-bold tracking-wider">
                  Daily Target
                </p>
                  ...
                  {isLoadingRequirement ? (
                    ...
                    <>
                      <span className="text-xl font-bold text-[#0A4D68]">
                        {formatKg(dailyRecommended)}
                      </span>
                      <span className="text-sm font-medium text-[#0A4D68]/70">
                        kg / day
                      </span>
                    </>
                  )}
              ...
              <div className="col-span-2 pt-2 border-t border-[#088395]/10">
                <p className="text-[#0A4D68]/60 text-[10px] uppercase font-bold tracking-wider mb-1">
            ...
            {tankBatches.length === 0 ? (
              <div className="bg-amber-500/10 border border-amber-500/20 p-4 rounded-xl text-amber-200">
                ...
                <p className="text-[11px] leading-relaxed">
                  You cannot record feeding for a tank without active batches.
                  Please stock this tank before adding feeding logs.
                </p>
              </div>
            ) : (
                  ...
                  <SelectContent>
                    {tankBatches.map((b) => (
                      <SelectItem key={b.id} value={b.id.toString()}>
                        Batch {b.batchNumber || b.id.toString().substring(0, 8)}{" "}
                        (
                        {(
                          b.counts?.current ??
                          b.currentCount ??
                ...
                <SelectContent>
                  {availableFoodTypes.length === 0 && !isLoadingFoodTypes ? (
                    <SelectItem value="none" disabled>
                      No matching food types found
                    </SelectItem>
                  ) : (
                    availableFoodTypes.map((ft) => (
                      <SelectItem key={ft.id} value={ft.id}>
                        {ft.name} ({ft.proteinPercentage}% Protein,{" "}
                        {ft.pelletSizeMm}mm)
                      </SelectItem>
                    ))
                  )}
            ...
            <div className="space-y-2">
              ...
              <div className="relative group">
                <Input
                  ...
                  value={weightFed}
                  onChange={(e) =>
                    setWeightFed(parseWeightValue(e.target.value))
                  }
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5 shadow-inner">
            <div className="flex items-center justify-between mb-4">
              ...
              <Badge
                className={`${progressPercent >= 100 ? "bg-green-500" : "bg-[#088395]"} text-[10px] uppercase font-bold`}
              >
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                <div className="flex justify-between text-[11px] font-medium text-gray-400">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span>
                      Progression: {formatKg(displayProgressFed)} /{" "}
                      {formatKg(dailyRecommended)} kg
                    </span>
                  </div>
                </div>
              ...
              {showUnderfeedingWarning && (
                <div className="flex items-start gap-3 bg-amber-500/10 border border-amber-500/20 p-3 rounded-xl text-[10px] text-amber-200 leading-relaxed shadow-sm">
                  ...
                  <p>
                    <strong className="text-amber-500">Caution:</strong>{" "}
                    Currently underfeeding relative to optimal growth
                    projection. If appetite is low, verify water parameters
                    immediately.
                  </p>
                </div>
              )}
          ...
          <div className="grid grid-cols-2 gap-3 pt-4 border-t">
            <Button
              ...
              onMouseEnter={(e) => {
                ...
                e.currentTarget.style.boxShadow =
                  "0 10px 25px rgba(220,0,0,0.35)";
              }}
              onMouseLeave={(e) => {
                e.currentTarget.style.background = "#008BFF";
            ...
            <Button
              ...
              style={{
                background: isSaving ? "#1DCD9F" : "#3B7597",
                color: "#fff",
                transition: "all 0.3s ease",
                padding: "25px",
                fontWeight: "bold",
              ...
              onMouseEnter={(e) => {
                if (!isSaving) {
                  e.currentTarget.style.background = "#1DCD9F";
                  e.currentTarget.style.transform = "translateY(-4px)";
                  e.currentTarget.style.boxShadow =
                    "0 10px 25px rgba(29,205,159,0.35)";
                }
              }}
              onMouseLeave={(e) => {
                if (!isSaving) {
                  e.currentTarget.style.background = "#3B7597";
                  e.currentTarget.style.transform = "translateY(0)";
                  e.currentTarget.style.boxShadow = "none";
                }
              }}
