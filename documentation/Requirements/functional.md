...
**Description:**
The system shall authenticate users using email/phone and password, with mandatory Two-Factor Authentication (2FA) for Admin and Farm Manager roles.
...
**Description:**
The system shall enforce role-based permissions dynamically, restricting user access to features and data based on assigned roles.
...
**Description:**
The system shall display a unified dashboard aggregating real-time data from multiple farms including weather forecasts, feed consumption, alerts, growth metrics, and market prices.
...
- Dashboard displaying:
  - Weather forecast (7-day, hourly updates)
  - Feed consumption graphs (daily/weekly with 30-day average comparison)
  - Active alerts list with severity indicators
  - Growth metrics charts (FCR, SGR, WG) with historical comparison
  - Water quality summary per tank
  - Current market prices with trend indicators
  - Interactive map with color-coded tank health status
...
**Description:**
The system shall continuously monitor water quality parameters from IoT sensors and generate alerts when values exceed species-specific safe ranges.
...
4. System retrieves species-specific safe ranges:
   - pH: 6.5-8.5 (varies by species)
   - DO: >5 mg/L (critical if <3 mg/L)
   - Ammonia: <0.5 mg/L
   - Temperature: species-dependent (e.g., 25-30°C for Tilapia)
   - Salinity: species-dependent
   - Turbidity: <40 NTU
5. System compares reading against threshold ranges
6. If value outside safe range, system calculates severity level:
   - Critical: DO <3 mg/L, pH <6.0 or >9.0, Ammonia >2 mg/L
...
10. Notification Service consumes event and sends alerts based on severity:
    - Critical: SMS, Email, WhatsApp, Dashboard
...
**Description:**
The system shall record bi-weekly fish weight samples and automatically calculate Feed Conversion Ratio (FCR), Specific Growth Rate (SGR), and Weight Gain (WG) with trend visualization.
...
4. System validates data:
   - Sample size ≥ 20 fish (or 10% of stock, whichever is greater)
   - Weight values are positive and realistic for species
   - Sampling date is within acceptable window
...
8. System calculates metrics:
   - **FCR = Total Feed Given (kg) / Weight Gain (kg)**
...
10. System compares metrics against:
    - Expected values for species and age
...
- Visual charts displaying:
  ...
  - FCR comparison (current vs. expected)
  - SGR trend with acceptable range highlighted
...
**Description:**
The system shall track operational expenses per tank and general farm expenses, automatically allocate shared costs, and generate comprehensive financial reports exportable to Excel/PDF.
...
**Input:**
- Expense details:
  - Type: Feed, Medicine, Labor, Fuel, Electricity, Maintenance, Other
  - Amount (EGP, USD, or other currency)
  - Date and invoice number
  - Tank ID (for tank-specific) or Farm ID (for general expenses)
...
5. For general expenses, system calculates allocation:
   - Retrieves production percentage per tank from Growth Records DB
   - Distributes expense proportionally (e.g., 40% to high-output tank)
   - Allows manual adjustment by Accountant
...
7. System calculates totals:
   ...
   - Category breakdown (feed, labor, etc.)
...
10. System calculates:
    - Total costs
    - Revenue (from production records if available)
...
**Description:**
The system shall track feed, medicine, equipment, and fuel inventory with automated depletion predictions and multi-stage expiry alerts.
...
**Input:**
- Item details:
  - Type: Feed, Medicine, Equipment, Fuel
  - Name and description (e.g., "Protein Feed 30%, 25kg")
  ...
  - Supplier ID
  - Storage location
  - Reorder threshold (default: 15% of maximum stock)
...
3. System initiates continuous monitoring:
   - Tracks daily consumption from feed logs and treatment records
   - Calculates moving average consumption (30-day window)
   - Adjusts for seasonal variations (higher consumption in summer)
4. System predicts depletion date:
   - Formula: Depletion Date = Current Date + (Remaining Quantity / Average Daily Consumption)
   - Updates prediction daily
5. System monitors expiry dates:
   - Checks daily at 8:00 AM
   - Identifies items expiring in 30, 15, and 7 days
6. System generates alerts:
   - **30 days before expiry:** Email notification
   - **15 days before expiry:** Email + Dashboard notification
   - **7 days before expiry:** Email + Dashboard + SMS notification
   - **Expired items:** Critical alert, flag for disposal
7. System checks stock levels:
   - If quantity ≤ reorder threshold (15%), triggers low stock alert
   - Includes supplier contact info and suggested reorder quantity
8. System provides reorder recommendations:
   - Based on consumption trend
   - Considers lead time from supplier
...
**Description:**
The system shall analyze uploaded fish images using a Convolutional Neural Network (CNN) to detect diseases, identify symptoms, and recommend treatments with confidence scores.
...
8. System calculates confidence scores:
   - Weighted confidence = Probability × Historical Accuracy
   - Considers prediction entropy
9. System applies confidence threshold validation:
   - If confidence ≥75%: Proceed with automatic diagnosis
   - If confidence <75%: Flag for expert review
...
12. System generates treatment recommendation:
    - Medication name and dosage (e.g., "10ml antibiotic per 100L water")
...
**Description:**
The system shall deliver time-sensitive alerts and notifications via multiple channels (SMS, Email, WhatsApp) based on event severity and user preferences.
...
**Input:**
- Event details:
  - Event type: Water Quality Alert, Mortality Alert, Low Stock, Disease Detection, System Error
  - Severity: Critical, Warning, Info
  - Source: Tank ID, Farm ID, or System
  - Message content
  - Timestamp
- User preferences:
  - Preferred channels per event type
  - Quiet hours (e.g., no SMS 10 PM - 7 AM)
  - Alert frequency limits

**Process:**
1. System event occurs (e.g., low DO detected, high mortality, stock depleted)
2. System determines event severity:
   - **Critical:** Immediate action required (DO <3 mg/L, 5%+ mortality, system failure)
   - **Warning:** Attention needed (values approaching limits, low stock)
   - **Info:** General updates (harvest ready, report generated)
3. System queries Notifications DB for similar recent alerts
4. System applies alert throttling:
   - Max 1 alert per parameter per tank every 15 minutes (for repeated conditions)
   - No throttling for escalating conditions
...
6. System determines delivery channels based on severity and preferences:
   - **Critical:** SMS + Email + WhatsApp + Dashboard (all channels)
   - **Warning:** Email + Dashboard
   - **Info:** Dashboard only (unless user opts in)
7. System checks quiet hours:
   - If within quiet hours and not critical, delay until quiet hours end
   - Critical alerts bypass quiet hours
8. System formats message for each channel:
   - SMS: Short, actionable (max 160 characters)
   - Email: Detailed with context and recommendations
   - WhatsApp: Medium detail with clickable dashboard link
   - Dashboard: Full details with visual indicators
9. System delivers notifications:
   - Sends SMS via Twilio API
   - Sends Email via SendGrid API
   - Sends WhatsApp via Twilio WhatsApp Business API
   - Publishes to Dashboard via WebSocket
...
12. System escalates if critical alert unacknowledged:
    - After 10 minutes, escalate to Admin
    - After 20 minutes, escalate to backup contacts
